...
/* 判断两个Bbox是否重合 */
export var isBboxOverLapping = function (b1, b2) { return (Math.abs(b1.centerX - b2.centerX) * 2 < b1.width + b2.width
...
export var getBBoxOfPoints = function (points, offset) {
    ...
    var minX = Math.min.apply(Math, __spread(xList));
    var maxX = Math.max.apply(Math, __spread(xList));
    var minY = Math.min.apply(Math, __spread(yList));
    var maxY = Math.max.apply(Math, __spread(yList));
    var width = maxX - minX;
...
/* 获取点的x,y方向上与box的交点 */
export var getBBoxCrossPointsByPoint = function (bbox, point) { return (getBBoxXCrossPoints(bbox, point.x).concat(getBBoxYCrossPoints(bbox, point.y))); };
/* 计算两点之间的预测距离(非直线距离) */
export var estimateDistance = function (p1, p2) { return (Math.abs(p1.x - p2.x) + Math.abs(p1.y - p2.y)); };
/* 减少点别重复计算进距离的误差 */
...
/* 预估距离 */
export var heuristicCostEstimate = function (p, ps, pt, source, target) { return estimateDistance(p, ps)
...
export var getPolylinePoints = function (start, end, sNode, tNode, offset) {
    ...
    if (isBboxOverLapping(sxBBox, txBBox)) {
        ...
        return __spread([start, sPoint], points, [tPoint, end]);
    }
...
var isOnLine = function (point, start, end) { return (point.x === start.x && point.x === end.x)
...
export var getBezierControlPoints = function (_a) {
    ...
    var sNext = getExpandedBBoxPoint(sExpendBBox, start);
    var ePre = getExpandedBBoxPoint(tExpendBBox, end);
    return { sNext: sNext, ePre: ePre };
};
...
export var getEndTangent = function (path) {
...
// 规范边初始化数据
export var pickEdgeConfig = function (data) { return pick(data, [
    'id',
    'type',
    'sourceNodeId',
    'sourceAnchorId',
...
export function createEdgeGenerator(graphModel, generator) {
    if (typeof generator !== 'function') {
        return function (sourceNode, targetNode, currentEdge) { return Object.assign({
